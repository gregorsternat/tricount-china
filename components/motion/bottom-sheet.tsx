...
export function BottomSheet({
  ...
  useEffect(() => {
    if (!open) return;
    const frame = window.requestAnimationFrame(() => setSnap(defaultSnap));
    return () => window.cancelAnimationFrame(frame);
  }, [open, defaultSnap]);
  ...
  useEffect(() => {
    ...
    const inerted = Array.from(document.body.children)
      .filter((element) => !element.hasAttribute("data-beui-bottom-sheet"))
      ...
      {open ? (
        <PresenceGate key="sheet">
          {({ gate }) => (
            ...
            >
              <div className="relative flex flex-col items-center px-4 pb-2 pt-3">
                ...
                {title || description ? (
                  <div className="mt-2 w-full pr-12">
                    {title ? (
                      <h2
                        id={titleId}
                ...
                <button
                  ...
                  onClick={() => onOpenChange(false)}
                  className="absolute right-3 top-3 grid size-11 place-items-center rounded-xl text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
